...
- ✅ **현행 서버 모델 = `gemini-2.5-flash-lite` 확정(2026-09-14).** `supabase secrets list` 에 `VERTEX_MODEL` 이
  **아예 없다** → 코드 기본값(`_shared/gemini-vertex.ts:11`)으로 돈다. 지금 원가는 위 표의 **2.5 행(₩0.035)**이다.
  3.1 전환(늦어도 10/2, `docs/pricing-decision.md`)은 이 시크릿 하나를 넣는 일이고, 그때부터 3.1 행이 된다.
- 📊 **첫 실측(2026-09-13)** — 외부 Free 계정 1명(가입 3일)이 하루 한도 50/50 을 다 쓰고 **광고는 0회** 본 뒤
  basic **37건**을 받았다. 그중 Vertex 호출 7건(캐시 히트 81%) → **그날 이 경로 원가 ≈ ₩0.25**.
  한 명·하루라 분포가 아니다 — §1단계 판단은 9/26 이후 그대로 한다.
...
✅ **스토어 원고 확인 완료(2026-09-12)** — 「한도를 넘겨도 뜻은 나온다」는 **어느 원고에도 약속으로
적혀 있지 않다.** 출시본 `appstore-1.6.3.txt` 와 다국어 `listing/*.md` 에는 "뜻만" 이 아예 없고,
1.6.2 원고의 "한도를 넘겨 담느라 뜻만 들어온 단어…를 한 번에 채워드려요" 는 **채우기 기능 설명**이지
basic 이 계속 나온다는 보장이 아니다(상한을 걸면 그런 단어가 덜 생길 뿐 문장은 여전히 참이다).
앱 문구(`addWord.basicQuotaExceeded`, `features/quota/basic-notice-copy.ts`)도 basic 이 실제로 왔을
때만 뜨므로 거짓이 되지 않는다. 🔑 **2단계에서 이 조사를 다시 하지 말 것** — 원고를 새로 쓸 때만 본다.
