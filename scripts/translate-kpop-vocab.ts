/**
 * K-pop / 아이돌 팬덤 용어 100을 영어 뜻 + 로마자 + 캐주얼 한국어 예문 + 영어 번역으로 enrich.
 * 방향: ko → en (영어권 K-pop 팬 대상). integrate-vocab.ts 의 ko→en 분기로 통합.
 *
 * 입력: scripts/kpop-source.json ({rank, term, pos, category, hint})
 * 출력: scripts/kpop-translated.json
 * 진행 파일: scripts/.kpop-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-kpop-vocab.ts
 * 옵션:
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/kpop-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/kpop-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.kpop-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert tutor of K-pop / idol fandom slang for English-speaking K-pop fans (stans).

Each input item is a word or phrase used in Korean K-pop fandom — fan culture, merch, music activities, member positions, idol-fan content, and fan slang. Many are recent neologisms or abbreviations. The "context" field gives an English gloss to pin down the fandom meaning. Trust the context.
...
Each item:
{
  ...
  "meaningEn": "Clear English meaning in K-pop fan terms. Use the equivalent English stan slang when one exists (e.g. \\"bias\\", \\"fancam\\", \\"comeback\\", \\"all-kill\\"). 1-2 senses.",
  "romaja": "Revised Romanization of the term. e.g. \\"deokjil\\", \\"choe-ae\\", \\"keombaek\\"",
  "exampleKo": "A natural, casual Korean sentence as a real fan would say it online or in chat, using the term. Use friendly modern register (해요체 or 반말, with common fandom tone). 8-18 words, Hangul only.",
  "exampleEn": "Natural, casual English translation of exampleKo (how an English-speaking stan would actually say it)."
}

Rules:
- Return EXACTLY ${batch.length} items, same order as input.
- Do NOT change the term field — copy exactly.
- The example MUST actually use the term and sound like real fan talk, not a stiff textbook sentence.
- meaningEn must reflect the K-pop fandom sense from "context", never an unrelated everyday meaning.
