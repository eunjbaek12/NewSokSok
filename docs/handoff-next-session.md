...
**✅ 통과 (7/10 사용자 확인, 12건):**
- 암기 분류 유실(#47) · 예문 넘침+상단 여백(#44) · 품사 필터(#37) · 홈 StatsStrip(#42) · 언어쌍 표시(#43) · 중도이탈 복습 기록(#36) · 자랑하기 공유시트 · 예문 오디오(#15) · 동음이의어 칩 동작(#41 — 피드백 2건으로 #49 토글 개편, 재검증은 build 27) · **기기 간 동기화 push→pull**(멀티디바이스 인시던트로 확인, 아래 세션 기록 참조) · **① 재설치 동기화 E2E**(#35 스트릭·달력·날짜별 외운 단어 복원 + #29 pull 대량 동시 커버 — 잔여 실기 검증 중 최고 위험 항목 해소) · **② 커뮤니티 공유(#46)**(공유→수신 E2E 통과 — #46 종결)
...
- [ ] **③ 비영어 3종 세트(한 흐름)**: vi/es 단어장에서 단어 추가 AI 검색 → 발음 표기(#20) → 저장 후 편집 열어 레이블·"단어 검색" 오탐 없음(#19) → N 버튼이 해당 언어쌍 사전으로(#22)
  - ⚠️ ③ 진행 중 **신규 버그 발견·수정(PR #52)**: 개인 단어장(+버튼)은 `createList`가 언어를 저장 안 해 학습 화면 TTS 4곳이 en-US 폴백 → 스페인어 예문·단어가 영어 음성으로 읽힘. `getStudySourceLang`(리스트 우선→단어 폴백, 오염 덱 보호) 신설. **재검증은 build 27**. build 26에선 큐레이션 덱(es/vi 저장분)으로 학습 TTS 우회 확인 가능
  - ⚠️ ③ 진행 중 **2호: vi 발음 "ㅓㅓ" 신고 → 수정(PR #53)**: 정체=IPA Chao 성조 막대 ˧˧(U+02E7, 폰트에서 ㅓ와 동일 렌더링). vi 철자가 성조 완전 표기라 중복 → 프롬프트 4곳 '성조 막대 없이'+`stripToneBars` 가드 3곳(enrichWord·AI생성·rowToWord, 강세 ˈˌ 보존)+PROMPT_VERSION 5 bump. **✅Edge v5 재배포+서버 E2E 통과(ɗi·caw 막대 0, 강세 보존, v4캐시 재생성)**. 기존 저장 단어 표시 정리는 build 27부터(mapper)
...
- [ ] **개인 단어장 학습 TTS 언어**(PR #52): +버튼 단어장에 es/vi 단어 추가 → 카드학습 단어·예문 발음이 해당 언어 음성인지(기존엔 en-US). 기존 저장 큐레이션 덱(vi/es) 학습 발음이 여전히 정상인지도 같이(리스트 우선 회귀 방지)
- [ ] **vi 발음 성조 막대 제거**(PR #53): vi 단어 AI 검색 → 발음에 "ㅓㅓ"(˧˧) 없음 + **기존 저장 vi 단어**도 표시에서 막대 사라짐(rowToWord 정리) + en 단어 강세 기호 ˈ 유지
