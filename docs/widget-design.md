...
| **단어장 이름** | 윗줄 = **칩 + 단어장 이름**(말줄임), 개수 «4 / 10»은 뜻 아래 오른쪽 11px 회색 | 위젯을 여러 개 놓고 따로 설정하는 것이 W7 의 전제(토익 위젯·회화 위젯)인데, 이름이 없으면 **두 위젯이 똑같이 생겼다**. 한 줄에 셋을 넣는 안은 칩 48 + 개수 34 + 여백 12 를 뺀 **52dp** 만 남아 이름이 두세 글자에서 잘린다(있으나 마나). 개수는 매 순간 볼 값이 아니라 «오늘 얼마나 남았나»를 가끔 보는 값이라 아래 줄이 어울린다. ⚠️ 큰 글자에서 뜻과 개수가 겹칠 수 있다 — «다시 볼게요» 큰 글자 실기와 같이 본다 |
