...
export const ANNOUNCEMENTS: Announcement[] = [
  {
    // 1.8.0 은 **셋을 한꺼번에 내는** 릴리스다 — 시험지 · 단어 알림 · 친구에게 보내기.
    // 10/1 에 1.7.0 이 먼저 나가므로 완주 상장·스킨 셋은 여기 적지 않는다(아래 블록이 갖고 있다).
    //
    // 🔴 **날짜는 출시일로 맞출 것.** version 이 app.json 과 다르면 시트가 아예 안 뜬다.
    version: '1.8.0',
    date: '2026-10-15',
    items: [
      // 이번 릴리스의 얼굴. 경쟁 리뷰 16,342개에서 동종 앱 수요 1위였다(372건, 「학원 단어시험」
      // 문맥이 직접 정당화한다). 아이콘은 단어장 화면의 학습 방식 줄이 쓰는 것과 같다
      // (app/list/[id].tsx 의 document-text-outline).
      //
      // 「뜻을 보고 단어를 직접 적는다」까지 적는 이유: 이름만으로는 객관식 퀴즈와 구별되지 않는다.
      // 앱에 이미 퀴즈가 있어서 「시험지」가 그것의 다른 이름으로 읽힐 수 있다.
      { key: 'whatsNew.v180_1', icon: 'document-text' },
      // 홈 권유 카드는 적지 않는다 — 켜라고 말을 거는 카드 자체가 소식이 아니라,
      // 그 카드가 가리키는 «알림이 온다»가 소식이다. 카드는 보면 바로 안다.
      { key: 'whatsNew.v180_2', icon: 'notifications' },
      // 경로를 적는다(⋯ › 친구에게 보내기) — 단어장 목록의 ⋯ 메뉴는 열어 보지 않으면
      // 무엇이 들었는지 모르는 자리다. 아이콘은 그 메뉴 줄과 같은 종이비행기.
      { key: 'whatsNew.v180_3', icon: 'paper-plane' },
      // 1.6.0·1.6.2·1.6.3·1.7.0 과 같은 마지막 줄·같은 아이콘.
      //
      // 🔴 **「제보에서 시작했어요」는 붙이지 않았다** — 1.8.0 에는 제보에서 나온 항목이 없다
      //    (시험지는 경쟁 리뷰 분석, 알림은 8/19 실측에서 나왔다). 1.7.0 과 같은 판단이다.
      { key: 'whatsNew.v180_4', icon: 'chatbubble-ellipses' },
    ],
  },
  {
    // 1.7.0 은 10/1 피처링에 맞춰 **감춰 뒀던 둘을 여는** 릴리스다 — 완주 자랑하기와
    // 스킨 세 종. 브랜치가 v1.6.3 태그에서 떠서 그 밖의 코드 변화가 없고, 그래서
    // 「화면이 달라졌는가」로 걸러도 남는 것이 정확히 이 둘이다.
    //
    // 🔴 **손 흔들기를 껐다는 것은 적지 않는다**(1427fc7). 규칙상 「고친 결과로 화면이
    //    달라졌을 때」에 걸리긴 하지만, 그 흔들림을 이상하게 본 사람은 iPhone 1.6.3
    //    사용자뿐이고 나머지에게는 "그런 게 있었구나"만 남는다. 1.6.3 에서 잔버그
    //    넷을 뺀 것과 같은 판단이다.
    //
    // 🔴 **마지막 줄에 「제보에서 시작했어요」를 붙이지 않았다** — 1.7.0 에는 제보에서
    //    나온 항목이 하나도 없다. 없는 감사를 적으면 다음 릴리스의 그 문장이 값을 잃는다.
    version: '1.7.0',
    // 10/1 수동 출시. 🔴 version 이 app.json 과 달라지면 시트가 아예 안 뜬다.
    date: '2026-10-01',
    items: [
      // 완주를 테마보다 **위에** 둔다(은정님 2026-09-15). 1.6.3 의 맨 윗줄도 테마였어서,
      // 두 릴리스 연속으로 테마가 머리에 오면 「또 테마?」로 읽힌다. 완주는 그에 비해
      // 사람이 하는 일 자체를 바꾼다 — 계획을 끝까지 가게 만드는 쪽이다.
      //
      // 두 줄로 나눈 이유: 기록(022·023)은 플래그가 닫힌 동안에도 쌓였으므로 **켜는 날
      // 이미 상장이 들어 있는 사람이 있다**. 그들에게 바로 다음 질문은 「어디서 다시
      // 보나」이고, 그 답이 두 번째 줄이다. 아이콘은 완주 보관함 줄의 리본 도장과 같은
      // 모양을 쓴다(app/stats.tsx 의 doneRibbon).
      { key: 'whatsNew.v170_1', icon: 'ribbon' },
      // 상장이 하나도 없는 사람에게는 「완주한 단어장」 줄 자체가 안 그려진다
      // (stats.tsx: completions.books > 0). 그래서 「받은 상장은」으로 시작한다 —
      // 받은 적이 없으면 찾아갈 일도 없다는 뜻이 문장 안에 들어가 있어야 한다.
      { key: 'whatsNew.v170_2', icon: 'albums' },
      // 1.6.3 의 테마 줄과 같은 아이콘·같은 틀. 「설정 › 화면 › 테마」 경로까지 그대로다.
      { key: 'whatsNew.v170_3', icon: 'color-palette' },
      // 🔴 **이 줄은 앱이 아니라 서버가 지킨다.** 덱 셋은 10/1 에 `--publish` 로 열리고,
      //    그 전에 이 소식이 먼저 나가면 「단어 모음에 새로 올라왔어요」가 거짓이 된다.
      //    출시와 덱 공개는 **같은 날 같이** 해야 한다(store-assets/event-cards/README.md).
      //
      // 🔴 **스페인어에서는 이 줄이 빈 문자열이라 시트에서 내려간다.** 단어 모음은 뜻
      //    언어로 거르는데 스페인어 뜻 덱이 0개라, 스페인어로 보는 사람에게는 이 덱이
      //    애초에 목록에 없다 — 적으면 찾을 수 없는 것을 약속하는 셈이다.
      //    (ko 는 「할로윈 영어 50」, en 은 「Korean Ghosts & Folk Horror 50」으로 갈린다.
      //     같은 날 열리는 덱이지만 뜻 언어가 서로 반대라 보이는 쪽이 다르다.)
      { key: 'whatsNew.v170_4', icon: 'school' },
      // 1.6.0·1.6.2·1.6.3 과 같은 마지막 줄·같은 아이콘.
      { key: 'whatsNew.v170_5', icon: 'chatbubble-ellipses' },
    ],
  },
  {
...
/**
 * 그 로케일에서 실제로 보여줄 줄만 남긴다.
 *
 * **한 언어에서만 줄을 내리는 방법 = 그 언어의 문구를 빈 문자열로 두는 것.** 키를 아예
 * 빼면 안 된다 — 폴백이 걸려 **영어 문장이 대신 나온다**(i18n/index.ts 의 fallbackLng).
 * 빈 문자열은 i18next 가 그대로 돌려주므로(returnEmptyString 기본값 true) 여기서 걸린다.
 *
 * 첫 사례는 1.7.0 의 덱 줄이다. 단어 모음은 **뜻 언어**로 거르는데 스페인어 뜻 덱이
 * 0개라, 스페인어로 보는 사람에게 「새 덱이 올라왔어요」는 찾을 수 없는 것을 약속하는
 * 문장이 된다. 소식은 번들에 담겨 나가므로 나중에 고칠 수도 없다.
 */
export function visibleItems(
  announcement: Announcement | null | undefined,
  t: (key: string) => string,
): AnnouncementItem[] {
  return (announcement?.items ?? []).filter(item => t(item.key).trim() !== '');
}
