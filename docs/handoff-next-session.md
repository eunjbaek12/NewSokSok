...
**⬜ 남은 검증 (테스트 절차 포함):**
- [ ] **⑥ 스모크**: ✅게스트 배너 광고 통과(7/10) · ⚠️요금제 화면에서 **신규 버그 발견·수정(PR #58)**: 진입만으로 "결제 실패" 알림 = iOS 미완료 거래 자동 재생을 사용자 결제로 오인(게스트 401·만료 샌드박스 402). userInitiatedRef 침묵 정산+402 확정 거절 좀비 청소. 재검증 build 27. **남은 확인: 사진 스캔 1회 · 구독 상품 가격(월 ₩3,900/연 ₩35,900) 카드 표시 여부(알림에 가려 미확인일 수 있음)**
