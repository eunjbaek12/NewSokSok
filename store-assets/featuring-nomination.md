...
⏭️ **Play 는 다음 기회를 노린다.** 평점 3.0 이 먼저다(현재 0). 리뷰가 쌓이고 다음 큰
모멘트가 오면 **그때는 «신규 앱» 이 아니므로 리드타임이 3주로 줄어든다** — 요건은 오히려
느슨해진다. §4-C 본문은 그대로 두었다.
⚠️ 위는 폼 안내 문서 기준이다. **폼 첫 화면의 자격 문항을 직접 열어 확인하면 확정된다**(5분).
