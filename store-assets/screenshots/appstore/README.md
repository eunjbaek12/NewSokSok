...
App Store Connect 업로드용 10장 스크린샷 캡처·합성 가이드.

## 디렉토리

```
appstore/
├── config.json      — 10장 메타데이터 (캡션 ko/en + 캡처 힌트 + 팔레트)
...
│   ├── ko/ 01-home.png … 10-theme2.png
│   └── en/ 01-home.png … 10-theme2.png
└── final/
...
- **장수**: 10장 / 언어 (한 + 영 = 20장). App Store 최대 10장. Play는 8장(테마 2장 제외, playstore/ 참조).
...
| `06-review` | 홈 '오늘의 복습' 배너 | **"오늘의 복습 · N개 준비됨" 배너가 히어로**로 크게 | 🆕 1.2.0. 01-home과 구도 겹치지 않게 |
| `07-curation` | 단어 모음 | 큐레이션 카드들, **다국어 섞이게** | |
| `08-stats` | 내 학습(통계) | **연속 학습·달력·외운 단어** 보이게, 캐릭터 포함 | 🆕 1.2.0 |
| `09-theme1` | 설정→표시→스킨 | 스킨(예: 다크 고요) 적용 화면 | 일반 화면과 톤 대비 |
| `10-theme2` | 설정→표시→스킨 | theme1과 다른 스킨(예: Y2K) | 두 장이 대비되게 |

### 저장
캡처본을 아래 경로에 ID 그대로 저장 (한국어→ko, 영어→en):
```
raw/ko/01-home.png  …  raw/ko/10-theme2.png
raw/en/01-home.png  …  raw/en/10-theme2.png
```
...
node store-assets/screenshots/appstore/compose.mjs   # 6.9" 20장
python store-assets/screenshots/appstore/resize65.py # 6.5" 20장
```
...
- **6.9" 디스플레이** 탭 → `final/6.9inch/ko` 또는 `/en` 10장, **01→10 순서대로**
