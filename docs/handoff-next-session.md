...
## 🎯 다음 세션 최우선 = build 28 실기 검증 (아래 §3)

**🚀 build 28 = 1.1.4 / buildNumber 28 / 커밋 `273474b`** — 2026-07-11 `eas build --auto-submit`으로 빌드+ASC 업로드 성공(Build ID `0862eaac-652a-41da-9ba7-8d155217c487`). Apple 처리 5~10분 후 TestFlight 반영. #64 클라 변경분 + #68(혼합 덱 TTS) + #69(스킨 글자색·스캔 미등재·placeholder) 포함.
...
### 3. build 28 — 실기 검증 체크리스트 (⓪부터 순서대로)
- [ ] **⓪ 업그레이드 설치(27→28) 데이터 보존**: 단어장·내 학습 정상 표시.
- [ ] **① 혼합 언어 덱 TTS**(#68): 사진 스캔했던 ko→vi 덱(build 27에서 미스매치 났던 그 덱)으로 카드학습 → 한국어 단어는 한국어 음성, vi 단어는 vi 음성. 혼합 덱에서 무음이던 단어도 소리 남. + 그 덱의 단어장 카드에 언어쌍 표시가 숨겨짐(hasMixedLanguagePairs).
- [ ] **② +버튼 단일 언어 덱 TTS 회귀**(#52+#68): es/vi 단어만 든 개인 덱 학습 발음 정상 유지.
- [ ] **③ 스킨 선택기 글자색**(#69): 다크 고요 적용 상태에서 4개 카드 이름 모두 읽힘 ↔ 클래식 적용 상태에서 다크 고요 카드 이름 읽힘.
- [ ] **④ 사진 스캔 미등재 제외**(#69): vi 입력 설정 + 영어 사진 스캔 → 빈 카드 대신 "N개는 사전에 없는 단어라 제외했어요" 안내(전부 제외되면 언어 설정 확인 Alert). + placeholder가 "예문 한국어 해석" 아닌 언어 인지 라벨.
- [ ] **⑤ 일괄 추가 미등재 표시**(#69): 일괄 추가에 가짜 단어(asdfgh 등) 섞어 넣기 → 해당 카드에 "사전에서 찾지 못했어요" 표시(자동 삭제 아님).
- [ ] **⑥ #64 클라 변경분**: ko→ko 언어쌍에서 "예문 해석" 입력 필드 숨김 · 언어쌍 변경 시 확인 Alert 후 초기화(신규 모드만, term 유지) · 기존 저장 단어의 중복 예문 번역 표시 생략(플래시카드·상세·예문·오토플레이).
