...
>
> ✅ **2026-09-13 실행 확인.** 이 블록을 그대로 돌려 정본과 대조했다 — 공통 16열의 값이 전부
> 일치하고, 문서판에만 위 두 열이 더 나온다. Q15도 정본과 값까지 같다.
...
> 🔴 **새 표본으로 갈지 않는 이유: 이 저장소는 공개다.** 표본을 갱신하면 그날 사용자의 이름과
> 단어장 제목이 새로 공개되는 셈이라, 열 구성만 위 표로 안내하고 실물은 늘리지 않는다.
