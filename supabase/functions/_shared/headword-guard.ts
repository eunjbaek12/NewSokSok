...
//
// ⚠️ 클라이언트 utils/headword-guard.ts 의 복제본 — 수정 시 반드시 함께 갱신
//    (__tests__/headword-guard.test.ts 패리티가 두 구현의 일치를 검증).
//
// 🔴 서버는 normalizeHeadword 를 **표제어에 적용하지 않는다.** 옛 앱이 보낸
//    `1. apple` 을 조용히 고치면 사용자 기기에 저장된 표제어와 어긋난다 —
//    막아서 '찾지 못함'으로 되돌리는 쪽이 맞다. 정규화는 캐시 키에만 쓴다.
//
