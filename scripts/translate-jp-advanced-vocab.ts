/**
 * 일본어 고급(JLPT N1) 단어를 Gemini로 한국어 뜻·예문으로 enrich.
 *
 * 입력: scripts/jp-advanced-source.json (build-jp-jlpt-source.ts n1 산출)
 * 출력: scripts/jp-advanced-translated.json
 * 진행 파일: scripts/.jp-advanced-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-jp-advanced-vocab.ts
 * 옵션:
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/jp-advanced-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/jp-advanced-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.jp-advanced-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert Japanese vocabulary tutor for Korean advanced learners.

For each Japanese entry, provide Korean meaning and a natural JLPT N1-level Japanese example sentence with its Korean translation.
...
Each item:
{
  ...
  "meaningKr": "Korean meaning (1-3 senses, comma-separated). 예: \\"고찰하다, 검토하다\\"",
  "exampleJa": "Natural advanced Japanese sentence (12-22 words) using the term in context. Use kanji form if the input has kanji, otherwise kana. JLPT N1 grammar (e.g. ～にあたって, ～ざるを得ない, ～かたわら, ～にもかかわらず, ～ものを, 〜ゆえに, 〜とはいえ).",
...
- Example must be JLPT N1 difficulty (advanced vocabulary, formal/literary grammar, abstract concepts).
- Topics should fit advanced contexts: news, business, academic discourse, social issues, culture, abstract reasoning.
...
async function main() {
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-jp-jlpt-source.ts n1 먼저 실행.`);
    process.exit(1);
  }
