/**
 * 일본어 중급(JLPT N3) 단어를 Gemini로 한국어 뜻·예문으로 enrich.
 *
 * 입력: scripts/jp-intermediate-source.json (build-jp-jlpt-source.ts n3 산출)
 * 출력: scripts/jp-intermediate-translated.json
 * 진행 파일: scripts/.jp-intermediate-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-jp-intermediate-vocab.ts
 * 옵션:
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/jp-intermediate-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/jp-intermediate-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.jp-intermediate-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert Japanese vocabulary tutor for Korean intermediate learners.

For each Japanese entry, provide Korean meaning and a natural JLPT N3-level Japanese example sentence with its Korean translation.
...
Each item:
{
  ...
  "meaningKr": "Korean meaning (1-3 senses, comma-separated). 예: \\"받아들이다, 수용하다\\"",
  "exampleJa": "Natural intermediate Japanese sentence (10-18 words) using the term in context. Use kanji form if the input has kanji, otherwise kana. JLPT N3 grammar (e.g. ～ようになる, ～ば～ほど, ～によって, ～にとって, ～ように).",
...
- Example must be JLPT N3 difficulty (intermediate vocabulary, mid-level grammar).
- Topics should fit intermediate contexts: school/work, daily life, opinions, social situations.
...
async function main() {
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-jp-jlpt-source.ts n3 먼저 실행.`);
    process.exit(1);
  }
