...
### dev client 검증 체크리스트 — ✅ 2026-05-23 전부 통과
1. 구글: 닉네임 설정 → 로그아웃 → 재로그인 → **닉네임 복원** ✅
2. 게스트: 단어·닉네임 설정 → 로그아웃 → 다시 게스트 → **유지** ✅
3. 합치기 **반복**(게스트 단어→구글 합치기→로그아웃 반복) → Supabase 고아 count **0 유지**, conflict 숫자 정상 ✅

추가로 같은 세션에 **로그아웃 후 게스트 로그인 시 로그인 화면으로 튕기던 race**도 발견·수정·검증 완료(`1303a13`). 원인: `onAuthStateChange`의 `SIGNED_OUT` 핸들러가 비동기로 늦게 도착해 방금 세팅한 게스트 세션을 `none`으로 덮어씀. `mode==='google'` 가드로 차단.
