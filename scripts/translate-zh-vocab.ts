/**
 * 중국어 단어를 Gemini로 한국어 뜻·예문으로 enrich.
 *
 * 입력: scripts/zh-source.json (build-zh-source.ts 산출)
 * 출력: scripts/zh-translated.json
 * 진행 파일: scripts/.zh-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-zh-vocab.ts
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/zh-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/zh-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.zh-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert Mandarin Chinese vocabulary tutor for Korean learners.

For each Chinese entry (simplified characters), provide the Korean meaning and a natural beginner-level Chinese example sentence with its Korean translation.

Input:
${inputJson}

The "hintEn" field is a rough English gloss from a dictionary — it may show a secondary or surname sense. Judge the MOST BASIC, COMMON meaning of the word based on its characters, pinyin, and part of speech, not necessarily the hint.
...
Each item:
{
  ...
  "meaningKr": "Korean meaning (1-3 senses, comma-separated). 예: \\"사랑하다, 좋아하다\\"",
  "exampleZh": "Natural beginner Chinese sentence (6-14 characters) in simplified characters using the term. HSK 1-2 level grammar and vocabulary.",
...
- Example must be HSK 1-2 difficulty (basic vocabulary, simple grammar), simplified characters only.
...
async function main() {
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-zh-source.ts 먼저 실행.`);
    process.exit(1);
  }
