...
export const ANNOUNCEMENTS: Announcement[] = [
  {
    // 1.6.3 은 「테마 셋 손질」 하나짜리 릴리스다. 함께 들어간 잔버그 넷(탭바 스킨 ·
    // 발음 슬래시 중복 · 영어 뜻 잘림 · ⋯ 메뉴 두 줄)은 **적지 않기로 했다**(은정님
    // 2026-09-10). 규칙상 「고친 결과로 화면이 달라졌을 때」에 걸리긴 하지만, 겪지
    // 않은 사람에게는 "이런 문제가 있었구나"만 남는 쪽에 가깝다.
    //
    // 🚩 **10/1 몫은 한 줄도 넣지 않는다** — 완주 자랑하기 · 가을·한글·할로윈 스킨은
    //    구현이 끝났고 플래그 뒤에 있다. 여기 적으면 그날의 「새것」이 미리 소진된다.
    //    같은 이유로 캐릭터 손 흔들기도 이번엔 끄고 나간다(app/(tabs)/index.tsx).
    version: '1.6.3',
    date: '2026-09-10',
    items: [
      // 이번 릴리스의 얼굴. 배경 그림이 생긴 것과 이름이 바뀐 것은 **다른 사건**이라
      // 한 줄에 욱여넣지 않는다 — 이름 변경은 쓰던 사람이 "내 테마 어디 갔지" 하는
      // 자리라 그 자체로 한 줄을 받을 값이 있다.
      { key: 'whatsNew.v163_1', icon: 'image' },
      // 선택은 id('y2k')로 저장돼 있어 이름만 바뀌고 그대로 유지된다(constants/skins.ts:77).
      // 문구의 「쓰시던 테마는 그대로예요」가 그 사실이다.
      { key: 'whatsNew.v163_2', icon: 'color-palette' },
      // 마지막 줄은 1.6.0·1.6.2 와 같은 아이콘. 다만 **「제보에서 시작했어요」는 뺐다** —
      // 1.6.3 에는 제보에서 나온 항목이 없다. 없는 감사를 적으면 다음 릴리스의 그 문장이
      // 값을 잃는다.
      { key: 'whatsNew.v163_3', icon: 'chatbubble-ellipses' },
    ],
  },
  {
