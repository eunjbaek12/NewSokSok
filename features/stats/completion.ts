...
/**
 * 🚩 완주 자랑하기의 공개 플래그. 10/1 피처링 패키지에서 켠다 — 스킨의 `SKIN_LIST` 와
 * 같은 역할이고, 감추는 기준도 같다: **«완성»이 아니라 «공개 시점»**. 그 전에 나가는
 * 중간 릴리스에 이 기능이 설명 없이 먼저 뜨면 10/1 의 «새것»이 미리 소진된다.
 *
 * 끄면 사라지는 것은 **진입점 셋뿐**이다 — 홈 완주 시트의 「자랑하기」, 내 학습의 완주
 * 보관함 줄, 학습 결과의 축하 모달.
 *
 * 🔴 **기록(022·023)은 끈 동안에도 그대로 쌓인다.** 그래야 켜는 날 보관함이 비어 있지
 * 않고, 그 사이의 완주에도 `celebratedAt` 이 찍혀 있어 **몇 주 전 완주가 켠 직후
 * 축하로 튀어나오지 않는다**(023 이 막으려던 바로 그 증상이다).
 */
export const COMPLETION_SHARE_ENABLED = false;
