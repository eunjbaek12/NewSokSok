...
/**
 * ✅ 완주 자랑하기의 공개 플래그 — **1.7.0 에서 열었다**(2026-09-14). 스킨의 `SKIN_LIST` 와
 * 같은 역할이었고, 감추는 기준도 같았다: **«완성»이 아니라 «공개 시점»**. 그 공개 시점이
 * 10/1 피처링이라 여기서 연다.
 *
 * 켜면 돌아오는 것은 **진입점 셋**이다 — 홈 완주 시트의 「자랑하기」, 내 학습의 완주
 * 보관함 줄, 학습 결과의 축하 모달.
 *
 * 🔴 **기록(022·023)은 끈 동안에도 그대로 쌓였다.** 그래서 켜는 날 보관함이 비어 있지
 * 않고, 그 사이의 완주에도 `celebratedAt` 이 찍혀 있어 **몇 주 전 완주가 켠 직후
 * 축하로 튀어나오지 않는다**(023 이 막으려던 바로 그 증상이다). 이 설계가 실제로
 * 값을 하는 순간이 지금이다.
 */
export const COMPLETION_SHARE_ENABLED = true;
