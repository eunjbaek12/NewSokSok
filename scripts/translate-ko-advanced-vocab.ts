/**
 * 한국어 고급 단어를 Gemini로 영어 뜻·예문으로 enrich (외국인 학습자용, ko→en).
 *
 * 입력: scripts/ko-advanced-source.json (build-ko-advanced-source.ts 산출)
 * 출력: scripts/ko-advanced-translated.json
 * 진행 파일: scripts/.ko-advanced-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-ko-advanced-vocab.ts
 * 옵션:
 *   --limit=N     상위 N개만 처리 (smoke test용)
 *   --model=lite  gemini-2.5-flash-lite 사용 (별도 RPD 버킷, quota 초과 시 폴백)
 *
 * Intermediate와 출력 스키마 동일. 예문 난이도만 TOPIK 5-6.
 */
import fs from 'fs';
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/ko-advanced-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/ko-advanced-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.ko-advanced-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert Korean vocabulary tutor for English-speaking advanced learners.

For each Korean word, provide an English meaning, Revised Romanization, a natural advanced Korean example sentence, and its English translation.
...
Each item:
{
  ...
  "meaningEn": "English meaning (1-3 senses, comma-separated). e.g. \\"to carry out, to perform, to execute\\"",
  "romaja": "Revised Romanization of the term. e.g. \\"suhaenghada\\"",
  "exampleKo": "Natural advanced Korean sentence (10-18 words) using the term. TOPIK level 5-6 grammar and vocabulary.",
...
async function main() {
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-ko-advanced-source.ts 먼저 실행.`);
    process.exit(1);
  }
