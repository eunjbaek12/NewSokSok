// 🔴 zh 만 국기가 아니라 한자 글리프다. 되돌리지 말 것.
// 이 앱에서 zh 는 역할이 둘이고 서로 다른 중국어를 가리킨다:
//   · 뜻 언어 — 중국어권 사용자가 한국어를 배운다. App Store 등록 로케일은
//     번체(zh-Hant = 대만·홍콩·마카오)라 🇨🇳 는 대상 시장과 어긋난다.
//   · 출발어 — 한국인이 중국어를 배운다. 공식 덱 4개가 HSK·본토 슬랭이라
//     이쪽은 간체·본토가 맞고 🇹🇼 로 바꾸면 그 넷이 대신 어긋난다.
// 국기 하나로는 둘 다 맞출 수 없어 어느 쪽도 편들지 않는 「中」 를 쓴다.
// 제대로 가르려면 zh-Hans/zh-Hant 로 코드를 쪼개야 하고, 그건 TTS 맵·서버 덱의
// language 값·기존 사용자 단어 마이그레이션까지 번지는 별도 작업이다.
