...
`constants/tokens.ts`의 `FontSize` · `FontWeight`만 쓴다. 원시 숫자를 직접 적지 않는다.

| 토큰 | 값 | 용도 | 실측 |
|---|---|---|---|
| `FontSize.caption` | 11 | 배지 · 보조 안내 | 56 |
| `FontSize.label` | 12 | 라벨 · 메타 | 87 |
| `FontSize.small` | 13 | 부가 설명 · 링크 | 103 |
| `FontSize.body` | 14 | 본문 기본 · 목록 행 | 109 |
| `FontSize.bodyLg` | 15 | 강조 본문 · 부차 버튼 | 74 |
| `FontSize.action` | 16 | 주 버튼 · 큰 수치 | 61 |
| `FontSize.titleSm` | 17 | 워크플로 모달 헤더(§1.3) 전용 | 17 |
| `FontSize.title` | 18 | 화면 · 모달 제목 | 38 |
| `FontSize.titleLg` | 20 | 시트 제목 | 23 |

반경(§6)과 같은 방법으로 정했다 — `fontSize` 선언 658개에 서로 다른 값이 **29개**였고,
이 아홉 단이 그중 **86%**를 덮는다.

**두께는 크기가 아니라 역할에 묶는다.** 조합을 열어 두면 한 화면에 13/400 · 13/500 ·
13/600이 섞여 무엇이 강조인지 사라진다 — 실제로 13pt는 세 두께로 다 쓰이고 있다.

| 토큰 | 두께 | 언제 |
|---|---|---|
| `FontWeight.regular` | 400 | 읽는 문장 — 설명 · 본문 |
| `FontWeight.medium` | 500 | 라벨 · 링크 · **부차 버튼** |
| `FontWeight.semibold` | 600 | 버튼 · 수치 · 강조 |
| `FontWeight.bold` | 700 | 제목에만 |

### 9.1 소수점 값을 쓰지 않는다

통일성을 실제로 깨는 것은 산발적인 큰 값이 아니라 **소수점 여섯 종**이다 —
`12.5`(16곳) · `11.5`(8) · `13.5`(5) · `15.5`(2) · `14.5`(2) · `10.5`(2), 모두 35곳.

"12는 작고 13은 큰데" 하며 눈으로 맞춘 값이라 **다음 화면에서 재현되지 않는다.**
같은 자리에 같은 근거로 12.5를 다시 고를 수 있는 사람은 없다. 가장 가까운 단으로 흡수한다.

### 9.2 스케일 밖

`22`~`36`(25곳)은 통계 화면의 큰 수치, 온보딩 표제 등 **화면 고유의 값**이다.
재사용되지 않는 값을 토큰으로 만들면 이름만 늘어나므로 스케일에 넣지 않는다.

### 9.3 적용 현황

토큰은 새로 쓰는 코드부터 적용한다. 기존 658곳을 일괄 교체하지는 않는다 —
1pt만 바뀌어도 감기는 줄이 달라져, 이득에 비해 회귀 위험이 크다.

적용된 곳: `features/bare-words/BareWordsSheet.tsx`(첫 사례).
