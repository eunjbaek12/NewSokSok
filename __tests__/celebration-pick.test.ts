...
describe('✅ 완주 자랑하기 공개 플래그', () => {
  /**
   * **1.7.0(10/1 피처링)에서 열었다**(2026-09-14). 이 테스트는 「아직 닫혀 있다」로
   * 켜는 날 함께 걸리라고 있었고 — 스킨의 `skin-registry.test.ts` 와 같은 역할 —
   * 제 일을 했다. 이제는 반대를 지킨다: 진입점 셋(홈 시트 「자랑하기」 · 내 학습
   * 보관함 줄 · 학습 결과 축하 모달)이 조용히 다시 사라지지 않게.
   */
  it('열려 있다', () => {
    expect(COMPLETION_SHARE_ENABLED).toBe(true);
  });
});
