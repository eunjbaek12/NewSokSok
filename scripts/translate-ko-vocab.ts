/**
 * 한국어 단어를 Gemini로 영어 뜻·예문으로 enrich (외국인 학습자용, ko→en).
 *
 * 입력: scripts/ko-source.json (build-ko-source.ts 산출)
 * 출력: scripts/ko-translated.json
 * 진행 파일: scripts/.ko-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-ko-vocab.ts
 * 옵션: --limit N  → 상위 N개만 처리 (smoke test용)
 *
 * 주의: 다른 언어와 방향이 반대(target=en). 출력 필드:
 *   meaningEn(영어뜻), romaja(로마자), exampleKo(한국어예문), exampleEn(영어번역), pos(교정)
 */
import fs from 'fs';
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert Korean vocabulary tutor for English-speaking beginners.

For each Korean word, provide an English meaning, Revised Romanization, a natural beginner Korean example sentence, and its English translation.
...
Each item:
{
  ...
  "meaningEn": "English meaning (1-3 senses, comma-separated). e.g. \\"to eat, to have a meal\\"",
  "romaja": "Revised Romanization of the term. e.g. \\"saram\\"",
  "exampleKo": "Natural beginner Korean sentence (5-12 words) using the term. TOPIK level 1-2 grammar and vocabulary.",
...
async function main() {
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-ko-source.ts 먼저 실행.`);
    process.exit(1);
  }
