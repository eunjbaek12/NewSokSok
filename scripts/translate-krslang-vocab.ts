/**
 * 한국 Gen-Z / MZ 슬랭 100을 영어 뜻 + 로마자 + 캐주얼 한국어 예문 + 영어 번역으로 enrich.
 * 방향: ko → en (영어권 학습자·K컬처 팬 대상). integrate-vocab.ts 의 ko→en 분기로 통합.
 *
 * 입력: scripts/krslang-source.json ({rank, term, pos, category, hint})
 * 출력: scripts/krslang-translated.json
 * 진행 파일: scripts/.krslang-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-krslang-vocab.ts
 * 옵션:
...
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/krslang-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.krslang-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert tutor explaining Korean Gen-Z / MZ-generation slang to English-speaking learners and K-culture fans.
...
Each item:
{
  ...
  "meaningEn": "Clear English meaning in casual terms. Use the equivalent English slang when one exists (e.g. \\"left on read\\", \\"flex\\", \\"the talking stage\\", \\"FOMO\\"). 1-2 senses.",
  ...
  "exampleEn": "Natural, casual English translation of exampleKo (how an English-speaking peer would actually say it)."
}
...
- meaningEn must reflect the slang sense from "context", never an unrelated literal dictionary meaning.
