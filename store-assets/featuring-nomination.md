...
접근성도 이번에 제대로 손봤습니다. 아이콘만 있어 스크린리더가 "버튼"이라고만 읽던 컨트롤이 109개였는데 전부에 이름과 역할을 붙였고, 다섯 화면의 접근성 트리를 전수로 훑어 읽지 못하는 버튼이 0인 것을 확인했습니다. 화면 어느 지점을 잡아도 스와이프로 뒤로 가도록 바꿨습니다 — 왼쪽 가장자리 몇 픽셀에서만 되던 동작이라, 큰 화면에서는 한 손으로 닿지 않는 자리였습니다.

딱딱한 암기 앱들 사이에서 따뜻한 크림 톤과 아보카도 캐릭터로 손맛 있는 디자인을 지향했고, 이번에 스킨을 세 벌 더 그려 여덟이 되었습니다 — 단풍 지는 가을, 한지와 먹의 한글, 보랏빛 밤의 할로윈. 색만 바꾼 게 아니라 화면마다 배경 그림을 따로 그렸습니다. 그래서 10월에는 앱이 계절을 따라 옷을 갈아입습니다 — 한글날에는 한지 위에서, 월말에는 호박과 유령 사이에서 단어를 외웁니다.
...
10월 1일 업데이트의 머리는 완주 자랑하기입니다 — 단어장을 끝까지 외운 순간을 카드로 만들어 나눕니다. 단어 앱에서 가장 드문 순간이 "다 외웠다"인데, 그 순간이 늘 조용히 지나갔습니다. 아이콘만 있던 버튼 109개 전량에 스크린리더가 읽을 이름과 역할을 붙였고, 가을·한글·할로윈 스킨을 더해 여덟 벌이 됩니다 — 색만 바꾼 게 아니라 화면마다 배경 그림을 따로 그려서, 10월 한 달 앱이 계절을 따라 옷을 갈아입습니다.

단어 하나만 넣으면 AI가 발음·뜻·예문·동의어까지 카드를 완성하고, 사진 한 장이면 단어장이 통째로, 주제만 적으면 AI가 목록을 만듭니다. 본인 Gemini 키를 넣어 AI를 무제한 무료로 쓰는 BYOK 모드는 모바일 어휘 앱에서 사실상 유일합니다. 오프라인 우선이라 지하철에서도 그대로 돌아가고, Google 로그인과 Play 구독으로 기기 사이가 이어집니다.
```
