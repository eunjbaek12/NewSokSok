...
- **기간 만료 플랜 "다시 학습" 상태 복구**(2026-06-10, build 11 미포함) — 홈 "나의 학습"에서 기간 만료(overdue) 단어장을 다시 학습해도 계속 "기간 만료"에만 갇히고 완주해야만 풀리던 버그 수정. `computePlanStatus`를 "방치(7일+ 미학습) AND 종료일 지남 → overdue"로 게이팅 + 홈 "다시 학습"이 `restartPlan`으로 플랜 창 재anchor(진행도 보존). 단위 테스트 43개 통과(plan-engine.test.ts 시나리오 5종 포함)이나 **expo-sqlite 쓰기·sync는 jest로 못 잡아 실기 검증 필요**.

  **수동 검증 절차** (게스트 또는 로그인 무관, 단어장 1개에 단어 6~10개 + 단기 플랜 필요):
  1. **만료 상태 만들기**: 단어장에서 "계획하기"로 플랜 생성(예: 하루 2단어) → 일부만 학습 → 기기 날짜를 플랜 종료일 + 8일 이후로 변경(또는 종료일 짧은 플랜으로 방치) → 앱 재실행. 홈 "나의 학습"에 **"기간만료"** 빨간 칩 카드 확인.
  2. **다시 학습 (학습 전)**: 만료 카드의 **"다시 학습"** 탭 → 플랜 화면 진입 → 곧바로 뒤로가기 → 홈 카드가 **"학습하기"**(진행중)로 바뀌고 **마지막에 학습하던 Day부터** 이어지는지 확인 (Day 번호 보존).
  3. **학습 후**: 그 Day를 학습(플래시카드/퀴즈 완료) → 홈 복귀 → 카드가 **"추가학습"**(오늘 달성, 초록)으로 바뀌는지 확인. **"기간만료"로 되돌아가지 않아야 함.**
  4. **직접 진입 경로**: 다른 만료 단어장을 "다시 학습" 버튼 *없이* 단어장→플랜 화면으로 직접 들어가 학습 → 홈 복귀 → 역시 만료가 풀리고 진행중으로 복구되는지 확인.
