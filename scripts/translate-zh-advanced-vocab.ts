/**
 * 중국어 고급(HSK 5급) 단어를 Gemini로 한국어 뜻·예문으로 enrich.
 *
 * 입력: scripts/zh-advanced-source.json (build-zh-hsk-source.ts 5 산출)
 * 출력: scripts/zh-advanced-translated.json
 * 진행 파일: scripts/.zh-advanced-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-zh-advanced-vocab.ts
 * 옵션:
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/zh-advanced-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/zh-advanced-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.zh-advanced-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0, langRetry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert Mandarin Chinese vocabulary tutor for Korean advanced learners.

For each Chinese entry (simplified characters), provide the Korean meaning and a natural HSK 5-level Chinese example sentence with its Korean translation.
...
Each item:
{
  ...
  "meaningKr": "Korean meaning (1-3 senses, comma-separated). 예: \\"가설, 추측\\"",
  "exampleZh": "Natural advanced Chinese sentence in simplified characters using the term. STRICTLY 14-20 characters — never exceed 20, and do not go below 14. Formal/written register. You MAY use one HSK 5 pattern (不仅…而且…, 即使…也…, 无论…都…, 既然…就…, 通过…来…, 由…组成) but ONLY if it still fits in 20 characters.",
...
- Example must be HSK 5 difficulty (advanced vocabulary, formal/written grammar, abstract topics), simplified characters only.
- exampleZh MUST be written in Chinese characters. Never write it in Korean.
- Length rule is absolute: count the characters of exampleZh and keep it within 14-20. If a sentence runs long, shorten it — do not keep it because the grammar pattern is nice. If it runs short, it is too plain for HSK 5 — add substance, not filler.
- Topics should fit advanced contexts: news, academic discourse, business, social issues, culture, abstract reasoning.
...
async function main() {
  ...
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-zh-hsk-source.ts 5 먼저 실행.`);
    process.exit(1);
  }
