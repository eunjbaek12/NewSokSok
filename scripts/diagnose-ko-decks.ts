/**
 * constants/curationData.ts 의 ko→en 덱 전체를 공용 규칙으로 훑는다.
 *
...
const targets = decks.filter(d =>
  d.sourceLanguage === 'ko' && d.targetLanguage === 'en' &&
...
console.log(`📚 ko→en 덱 ${targets.length}개 검사\n`);
let totalHard = 0;
let totalWords = 0;

for (const d of targets) {
  // 앱 슬롯 → 검사용 형태. ko→en 덱은 뜻 슬롯이 영어, 원어 예문 슬롯이 한국어다.
  ...
  const label = `${d.title} (${entries.length}단어)`;
  if (!findings.length) {
    console.log(`✅ ${label}`);
    continue;
  }
