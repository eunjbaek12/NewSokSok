...
### 2. build 27 — 실기 검증 체크리스트 (⓪①②④⑤ 통과·⑥ 스킵, **잔여 = ③ 학습 TTS뿐**)
...
- **PR #69 — build 27 검증 중 신고 3건 수정**: ①스킨 선택기 글자색(다크 고요 ↔ 밝은 테마 양방향 가독) ②사진 스캔 사전 미등재(isReal=false) 카드 자동 제거+제외 안내(vi 설정+영어 사진으로 확인) ③사진/일괄 임포트 placeholder 언어화("예문 한국어 해석" 하드코딩 제거).
- **음성인식 첫 사용 오류(7/11 신고)**: 권한 허용 직후 1회 오류 알림 → 이후 정상 동작(사용자 확인). 일시 오류로 추정·수정 없음. 재발 시 팝업 문구 확보.
