...
### 2. build 27 — 실기 검증 체크리스트 (⓪ 통과, ①~⑥ 잔여 — 순서대로 하면 효율적)
...
- [ ] **① 동음이의어 v4 토글 칩**(#49): "눈"(ko→en) 검색 → 칩 토글·①② 조립 동작은 확인됨(그 과정에서 아래 3건 발견). **잔여 확인**: 전부 끄려 하면 최소 1개 힌트 · 저장 한도 초과 시 토글 거부 · 카드학습 TTS가 ①② 기호 안 읽는지 · **ko→ko 재검색 시 예문 번역 빈 값으로 나오는지**(서버 v6 — build 27에서 즉시 확인 가능).
...
### 3. build 28 대기분 (빌드 시 검증 목록에 추가)
- **PR #64 클라 변경분**: 같은 언어쌍(ko→ko)에서 "예문 해석" 입력 필드 숨김 · 언어쌍 변경 시 확인 Alert 후 초기화(신규 모드만, term 유지) · 기존 저장 단어의 중복 예문 번역이 표시에서 사라짐(플래시카드·상세·예문·오토플레이) · BYOK v6 프롬프트(같은 언어쌍·뜻 목록 고정).

### 4. 검증 통과 시 다음 트랙
- **1.1.4 App Store 프로덕션 제출 결정**: build 27로 낼지, PR #64 클라 변경분까지 실은 build 28로 낼지(①~⑥ 통과 여부와 #64 변경의 사용자 체감을 보고 판단. 제출 후엔 다음 버전은 1.1.5).
- Android 트랙도 같은 커밋으로 빌드 검토.
