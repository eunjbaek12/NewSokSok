...
| 9 | `08-theme` | **홈 탭** (스킨 적용 상태) | **duo** — `08-theme1`=다크 고요(뒤) + `09-theme2`=Y2K(앞) | 🥑 브랜드 개성 |
...
>   찍는 곳은 **스킨 설정 화면이 아니라 홈 탭**이다 — 스킨을 바꾼 뒤 홈으로 나와서 찍는다.
>   **앞뒤 테마를 ko·en에서 통일할 것**: `08-theme1`=다크 고요(뒤) · `09-theme2`=Y2K(앞).
>   밝은 쪽이 앞이라야 크림 배경과 이어진다. duo에서 뒤 폰은 왼쪽 절반쯤만 보인다.
