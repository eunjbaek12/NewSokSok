/**
 * 수능 필수 어휘 500을 한국어 뜻·예문·발음·품사로 enrich.
 *
 * 입력: scripts/suneung-source.json (build-suneung-source.py 산출, {rank, term, sfi, ngslRank, marker, definition})
 * 출력: scripts/suneung-translated.json
 * 진행 파일: scripts/.suneung-progress.json (중단 후 재실행 가능)
 *
 * 실행: npx ts-node scripts/translate-suneung-vocab.ts
 * 옵션:
...
const SOURCE_PATH = path.resolve(process.cwd(), 'scripts/suneung-source.json');
const OUTPUT_PATH = path.resolve(process.cwd(), 'scripts/suneung-translated.json');
const PROGRESS_PATH = path.resolve(process.cwd(), 'scripts/.suneung-progress.json');
...
async function translateBatch(batch: SourceEntry[], retry = 0): Promise<TranslatedEntry[]> {
  ...
  const prompt = `You are an expert English vocabulary tutor preparing Korean high school students for the 수능 (CSAT, Korea's college entrance English exam).

For each English word, provide Korean meaning, IPA, part of speech, and a natural example sentence at 수능 reading-passage level, with its Korean translation.
...
Each item:
{
  ...
  "phonetic": "IPA pronunciation, e.g. /əˈkɔːrd/",
  "pos": "noun | verb | adjective | adverb | phrase",
  "meaningKr": "Concise Korean meaning (1-3 senses, comma-separated). 예: \\"축적하다, 모으다\\"",
  "exampleEn": "Natural English sentence (12-20 words) at 수능 reading level — academic, abstract, or argumentative register (science, society, environment, psychology, philosophy, technology).",
...
- exampleEn should resemble the formal, analytical English of 수능/모의고사 reading passages (not casual conversation).
...
async function main() {
  if (!fs.existsSync(SOURCE_PATH)) {
    console.error(`❌ 소스 없음: ${SOURCE_PATH}. build-suneung-source.py 먼저 실행.`);
    process.exit(1);
  }
