...
export const ANNOUNCEMENTS: Announcement[] = [
  {
    // 🔴 **아직 미완성 — 1.8.0 을 낼 때 마저 쓴다.** 지금은 단어 알림 한 줄뿐이고,
    //    친구에게 보내기 · 시험지 · (되면) 위젯 줄이 더 붙는다. `date` 도 실제 출시일로
    //    바꿔야 한다. 그때까지는 app.json 이 1.6.3 이라 이 시트가 뜨지 않는다 —
    //    그게 이 파일이 기대는 안전장치다(위 ⚠️).
    //
    //    한 줄을 미리 적어 두는 이유: 문구를 기능과 함께 쓰면 «설정 › 단어 알림»이라는
    //    실제 경로가 눈앞에 있고, 릴리스 날 기억에 기대 다시 쓰지 않아도 된다.
    version: '1.8.0',
    date: '2026-10-15',
    items: [
      // 홈 권유 카드는 적지 않는다 — 켜라고 말을 거는 카드 자체가 소식이 아니라,
      // 그 카드가 가리키는 «알림이 온다»가 소식이다. 카드는 보면 바로 안다.
      { key: 'whatsNew.v180_1', icon: 'notifications' },
